# Data Model — 003 User Progress

Sigue las convenciones de seguridad de [[AGENTS.md]] (search_path, RLS con `(select auth.uid())`, `security definer` con EXECUTE revocado).

## Enum de fuente de XP

```sql
create type public.xp_source as enum ('lesson_complete', 'streak_bonus', 'achievement');
```

## `xp_events` (log inmutable)
...
-- Sin políticas de INSERT/UPDATE desde cliente: solo escribe el RPC (security definer).
```

## RPC `complete_lesson`

Hace todo en una transacción, con la XP calculada en el server.
...
declare
  ...
  v_xp int;
  v_today date := current_date;         -- fecha UTC del server (limitación documentada)
  v_last date;
  v_streak int;
  v_new_total int;
begin
  ...
  -- La lección debe existir y pertenecer a una unidad publicada.
  if not exists (
  ...
  -- XP calculada en el server (nunca se confía en el cliente).
  ...
  on conflict (user_id, lesson_id) do update set
    ...
    best_first_try_correct = greatest(public.lesson_completions.best_first_try_correct, excluded.best_first_try_correct),
  ...
  if v_last = v_today then
    v_streak := coalesce(v_streak, 0);              -- ya jugó hoy: sin cambios
  elsif v_last = v_today - 1 then
    v_streak := coalesce(v_streak, 0) + 1;          -- día consecutivo
  else
    v_streak := 1;                                  -- primer día o se cortó
  end if;
...
`apps/mobile/supabase/migrations/20260814000000_progress.sql`. Tras aplicar: advisors (security + performance) + regenerar tipos.
